...
st.markdown('Start date (yyyy-mm-dd)')
st.markdown('Choke Size (/64")')
...
st.markdown('Oil Rate (bbls/d)',)
st.markdown('Gas Rate (mmscfd)')
st.markdown('Water Rate (bbls/d)')
st.markdown('GOR (scf/d): Gas Oil Ratio')
st.markdown('Oil (bbls)')
st.markdown('Gas (mmscf)')
st.markdown('Water (bbls)')
st.markdown('BSW (%)')
...
if uploaded_file is not None:
    ...
    # Make prediction
    predict_batch(model=model, batch_input_features=X) 

    # Output data frame
    st.table(pd.DataFrame({'Start date': well_data['Start date'], 'Prediction': y_pred}))
    st.dataframe(pd.DataFrame({'Start date': well_data['Start date']}))
